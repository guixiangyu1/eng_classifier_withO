...
class NERModel(BaseModel):
    ...
    def run_evaluate(self, test):
        ...
        for words, labels in minibatches(test, self.config.batch_size):
            labels_pred, seqence_lengths = self.predict_batch(words)

            for lab, lab_pred, length in zip(labels,labels_pred,seqence_lengths):
                lab = lab[:length]
                lab_pred = lab_pred[:length]
                accs += [a == b for (a, b) in zip(lab, lab_pred)]
                precisions += [a==b for (a, b) in zip(lab, lab_pred) if self.idx_to_tag[a] != 'O']
        acc = np.mean(accs)
        ...
        #             f.write("0\n")
        # acc = np.mean(accs)
        print("correct_preds: ", acc*len(precisions))
        print("total_entity: ", len(precisions))
        print("accuracy: ", acc)

        return {"acc": 100*acc, "f1": precision*100}
