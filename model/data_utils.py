...
def CoNLLdata4classifier(dataset, processing_word=None, processing_tag=None):
    ...
    for raw_words, raw_tags in dataset:
        ...
        for i, (word, tag) in enumerate(zip(raw_words, raw_tags)):
            if tag == 'O':
                tags.append(processing_tag(tag))
                words.append(processing_word(word))


        for ne_type, ne_start, ne_end in chunks:
            for i in range(ne_start, ne_end):
                if i==ne_start:
                    entity = raw_words[i]
                else:
                    entity = entity + '$@&' + raw_words[i]
            words.insert(ne_start, processing_word(entity))
            tags.insert(ne_start, processing_tag(ne_type))
